...
namespace poly {
    ...
    void copy(int *f, int *g, int n) { for (int i = 0; i <= n; i++) f[i] = g[i]; }
    void clear(int *f, int n) { for (int i = 0; i <= n; i++) f[i] = 0; }
    void erase(int *f, int l, int r) { for (int i = l; i <= r; i++) f[i] = 0; }
    void reverse(int *f, int n) { for (int i = 0; i <= n / 2; i++) swap(f[i], f[n - i]); }
    ...
    void inverse(int *f, int *g, int n) {
        ...
        ntt(t, 1), ntt(f, 1);
        for (int i = 0; i < lim; i++) f[i] = f[i] * (2 - f[i] * t[i] % mod) % mod;
